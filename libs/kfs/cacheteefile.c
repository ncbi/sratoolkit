...
#define SET_CACHE_BIT( CacheFile, Block_Nr ) CacheFile->bitmap[ (Block_Nr) >> 3 ] |= BitNr2Mask[ (Block_Nr) & 7 ]
...
static rc_t create_bitmap_buffer( uint8_t ** bitmap, size_t bitmap_bytes )
{
    rc_t rc = 0;
...
static bool is_bitmap_full( const uint8_t * bitmap, size_t bitmap_bytes, uint64_t block_count )
{
...
static rc_t verify_file_structure( const uint64_t local_size, const uint32_t block_size, const uint64_t content_size,
                                   uint64_t * block_count, size_t * bitmap_bytes )
{
    rc_t rc = 0;
...
/*
    if ( rc == 0 && content_size != cf -> remote_size )
    {
        ...
        PLOGERR( klogErr, ( klogErr, rc, "content-size in local file $(ls) does not match size of remote file $(rs)", 
                           "ls=%lu,rs=%lu", content_size, self->remote_size ) );
    }
*/
...
static rc_t CC KCacheTeeFileDestroy( KCacheTeeFile *self )
{
    ...
    if ( rc == 0 && self -> fully_in_cache && ! self -> local_read_only && self -> lock != NULL )
        promote_cache( self );
...
static rc_t resize_cratch_buffer( const KCacheTeeFile *cself, size_t new_size )
{
    rc_t rc = 0;
...
{
    size_t res = bsize;
    if ( pos + bsize >= cself->remote_size )
    {
                                ...
                                void *buffer, size_t bsize, size_t *num_read )
{
    ...
    if ( bsize > 0 )
    {
        ...
        if ( rc == 0 && !cself->local_read_only )
            KFileWriteAll( cself->local, pos, buffer, bytes_read, num_read );
    }
    return rc;
}
                               ...
                               void *buffer, size_t bsize, size_t *num_read, uint64_t first_requested_block )
{
    ...
    if ( rc == 0 && cself->cluster_factor > 1 )
    {
        ...
        rc = resize_cratch_buffer( cself, over_read_size );
        if ( rc == 0 )
        {
            size_t tmp_read;
                               ...
                               uint64_t first_requested_block, uint64_t last_cache_miss_block )
{
    ...
    rc = resize_cratch_buffer( cself, t_len );
                               ...
                               void *buffer, size_t bsize, size_t *num_read )
{
    ...
    *num_read = 0;
    ...
    {
        rc = KCacheTeeFileRead_Starting_with_Cache_Hit( cself, pos, buffer, bsize, num_read, first_requested_block );
    }
    else
    {
        rc = KCacheTeeFileRead_Starting_with_Cache_Miss( cself, pos, buffer, bsize, num_read, first_requested_block );
    }
    ...
    uint32_t blocksize, uint32_t cluster, bool report, const char *path )
{
    ...
    else
    {
        ...
        else
        {
            if ( cf -> local_size == 0 )
            {
                /* the local file was just created... we have to ask the remote-file about it's size now! */
                rc = KFileSize( cf -> remote, &cf -> remote_size );
                if ( rc != 0 )
                    LOGERR( klogErr, rc, "cannot detect size of remote file" );
                else if ( cf -> remote_size == 0 )
                {
                    rc = RC ( rcFS, rcFile, rcConstructing, rcParam, rcInvalid );
                ...
                if ( rc == 0 )
                {
                    ...
                    if ( rc == 0 )
                    {
                        ...
                        if ( rc == 0 )
                        {
                            ...
                            if ( rc == 0 )
                            {
                                rc = KFileInit( &cf -> dad, (const union KFile_vt *)&vtKCacheTeeFile, true, false );
                                if ( rc == 0 )
                                {
